...
public class ChessGame {
    ...
    public ChessGame(Color turn, List<Board> history, Validator winnerValidator, Validator checkValidator){
        this.turn = turn;
    ...
    public GetResult<ChessGame, Boolean> move(Movement movement){
        ...
        if (board.getPieces().containsKey(movement.getOrigin())){
            ...
            if (piece.getColor() == turn && piece.getValidator().isValid(history, movement)){
                ...
                return new GetResult<>(Optional.of(new ChessGame(nextTurn(), newHistory, winnerValidator, checkValidator)), false);
            }
        }
